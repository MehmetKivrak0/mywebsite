...
export default function GitHubProjectsClient({ repos }: GitHubProjectsClientProps) {
  ...
  if (repos.length === 0) {
    return (
      <section className='w-full rounded-3xl border border-white/10 dark:border-dark_border bg-gradient-to-br from-white/8 via-white/4 to-transparent dark:from-darklight dark:via-darkmode dark:to-darkmode px-5 py-6 sm:px-7 sm:py-7 backdrop-blur-xl shadow-[0_18px_60px_rgba(0,0,0,0.35)] dark:shadow-darkmd'>
    ...
    <section className='w-full rounded-3xl border border-white/10 bg-gradient-to-br from-white/8 via-white/4 to-transparent px-5 py-6 sm:px-7 sm:py-7 backdrop-blur-xl shadow-[0_18px_60px_rgba(0,0,0,0.35)]'>
      <div className='flex flex-col gap-3 mb-5 sm:flex-row sm:items-end sm:justify-between'>
        <div>
          ...
          <p className='mt-1 text-xs sm:text-sm text-white/90 dark:text-darktext max-w-xl'>
            <Trans
      ...
      <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
        {visibleRepos.map((repo) => (
          <Link
            ...
            className='group relative flex h-full w-full flex-col overflow-hidden rounded-2xl border border-white/12 dark:border-dark_border bg-white/8 dark:bg-darklight p-4 sm:p-5 backdrop-blur-md shadow-[0_14px_40px_rgba(0,0,0,0.35)] dark:shadow-darkmd transition-transform duration-200 hover:-translate-y-1 hover:border-LightApricot/70 dark:hover:border-primary/70'
            style={{ aspectRatio: '1 / 1' }}
          >
            <div className='pointer-events-none absolute -right-2 -top-1 h-16 w-16 rounded-full bg-gradient-to-br from-white/10 dark:from-darkmode/50 to-transparent opacity-60 blur-xl' />
            <div className='pointer-events-none absolute right-2 top-2 flex h-8 w-8 items-center justify-center rounded-full border border-white/20 dark:border-dark_border bg-white/12 dark:bg-darkmode'>
              <Icon icon='mdi:github' className='text-white/75 dark:text-darktext text-lg' />
            </div>

            <div className='relative flex items-start gap-3 pb-3'>
              <div className='flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/20 dark:border-dark_border bg-white/10 dark:bg-darkmode'>
